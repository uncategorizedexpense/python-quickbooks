...
class QuickBooks(object):
    ...
    def get_report(self, report_type, qs=None):
        ...
        url = self.api_url + "/company/{0}/reports/{1}".format(
            self.company_id, report_type
        )
    ...
    def make_request(
        ...
        req = self.process_request(
            request_type, url, headers=headers, params=params, data=request_body
        )
    ...
    def get_single_object(self, qbbo, pk):
        url = "{0}/company/{1}/{2}/{3}/".format(
            self.api_url, self.company_id, qbbo.lower(), pk
        )
        result = self.get(url, {})
    ...
    def create_object(
        self, qbbo, request_body, _file_path=None, request_id=None, params=None
    ):
    ...
    def isvalid_object_name(self, object_name):
        if object_name not in self._BUSINESS_OBJECTS:
            raise Exception(
                "{0} is not a valid QBO Business Object.".format(object_name)
            )
    ...
    def download_pdf(self, qbbo, item_id):
        ...
        if response.status_code != httplib.OK:
            ...
            try:
                ...
                raise exceptions.QuickbooksException(
                    "Error reading json response: {0}".format(response.text), 10000
                )
